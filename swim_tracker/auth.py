...
def _normalized_team_name(name: str) -> str:
    ...
    if not (MIN_TEAM_NAME_LENGTH <= len(normalized) <= MAX_TEAM_NAME_LENGTH):
        raise ValueError(
            f"Team names must be {MIN_TEAM_NAME_LENGTH} to {MAX_TEAM_NAME_LENGTH} characters."
        )
    return normalized
...
def authenticate_team(target: str | Path, name: str, password: str) -> tuple[int, str] | None:
