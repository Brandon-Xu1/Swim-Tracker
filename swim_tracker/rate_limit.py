...
class RateLimitedError(Exception):
    def __init__(self, retry_after_seconds: float) -> None:
        super().__init__(f"Rate limited; retry in {retry_after_seconds:.0f} seconds.")
...
class SlidingWindowLimit:
    ...
    def retry_after(self, history: Sequence[float], now: float) -> float:
        ...
        recent = sorted(timestamp for timestamp in history if timestamp > now - self.window_seconds)
        if len(recent) < self.max_calls:
            return 0.0
        return recent[0] + self.window_seconds - now


def try_acquire(limits: Sequence[SlidingWindowLimit], history: list[float], now: float) -> float:
    ...
    history[:] = [timestamp for timestamp in history if timestamp > now - longest]
