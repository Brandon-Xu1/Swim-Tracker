    ...
    def test_app_starts_without_api_key(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(os.environ, environment, clear=False):
                ...
                self.assertTrue(any("3,999" in metric.value for metric in app.metric))

                next(field for field in app.text_input if field.label == "Swimmer name").set_value(
                    "Pierce Arora"
                )
                next(box for box in app.selectbox if box.label == "Event").select("100-yard Free")
    ...
    def test_deleted_data_stays_deleted_across_restarts(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(os.environ, environment, clear=False):
                ...
                self.assertTrue(any("3,999" in metric.value for metric in first_run.metric))
                ...
                self.assertTrue(any(metric.value == "0" for metric in second_run.metric))
